...
- **Status: ✅ done (2026-06-24).** Graph in `app/graph/{state,nodes,build}.py`. **Unified-agent shape** ([ADR-0009](../../decisions/0009-unified-agent-turn.md), superseding ADR-0007's split): `retrieve_context → agent(persona+tools) ⇄ tools(gate-backed) → write_memory`. ONE persona+tools agent both proposes tools and writes the reply; cap (3) enforced inside the agent (overflow turn drops tools → forced reply). `AsyncSqliteSaver` (langgraph 1.2.6) at `settings.checkpoint_path`, `thread_id=f"{npc_id}:{player_id}"`; durable windowed `history` survives restart. Gate rejections fold back as `ToolMessage`s the agent re-reasons over (subsumes ADR-0004's note). `tool_use_failed` cured at the schema layer (`int|str` coercion, [ADR-0008](../../decisions/0008-tolerant-numeric-tool-arg-schemas.md)). Persona tokens stream via `astream_events(version="v2")` filtered by the `persona` tag. **Live-verified vs Groq:** insult → `UpdateDisposition` fired + in-character reply; reward-for-unstarted-quest → refused in character, no reward granted. 45 offline tests pass (restart-continuity + scratch-no-leak + rejection + loop-termination). Re-reviewed (separate pass): 0 CRITICAL, both hard rules upheld; fixes folded (structural termination + recursion_limit backstop, empty-stream fallback, guarded DB-open, dead-state removal) — [ADR-0009](../../decisions/0009-unified-agent-turn.md) addendum.
